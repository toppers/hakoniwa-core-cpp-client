...
1. 箱庭アセットを登録する **[UC-HAKO-ASSET-SIM-1]**
2. 箱庭コア機能に対してシミュレーション実行を依頼する **[UC-HAKO-ASSET-SIM-2]**
3. 箱庭コア機能からのイベント（コールバック）として、箱庭アセットの処理を実行する **[UC-HAKO-ASSET-APP-1]**
4. シミュレーション終了まで 2-3 を繰り返す

アプリケーション固有の処理は、**[UC-HAKO-ASSET-APP-1]** であり、シミュレーションの必要なタイミングで、箱庭コア機能からコールバックされます。
箱庭アセット開発者は、このコールバック関数を実装することで、箱庭アセットとしてシミュレーションを実行できます。

ここで、コールバック処理としては、以下の３種類あります。

* 箱庭アセットの初期化処理 **[UC-HAKO-ASSET-APP-1a]**
  * 箱庭のシミュレーション開始時点で１回だけ呼び出されます。
* 箱庭アセットのシミュレーション実行処理 **[UC-HAKO-ASSET-APP-1b]**
  * 箱庭のシミュレーション時間ステップ毎に呼び出されます。
* 箱庭アセットのリセット処理 **[UC-HAKO-ASSET-APP-1c]**
  * 箱庭のシミュレーションがリセットされたタイミングで呼び出されます。

アプリケーション固有の処理実装では、アプリケーション共通の機能を利用できます。

* 箱庭PDUデータを読み込みする **[UC-HAKO-ASSET-APP-COMMON-1]**
* 箱庭PDUデータを書き込みする **[UC-HAKO-ASSET-APP-COMMON-2]**
* 箱庭シミュレーション時間を取得する **[UC-HAKO-ASSET-COMMON-3]**
* スリープする **[UC-HAKO-ASSET-COMMON-4]**
